...
class Datacenter:
    ...
    async def add_cell(self, cell_id: str, rpc_port: int,) -> dict:
        ...
        try:
            ...
            self.cell_locations[cell_id] = 'localhost' 
    ...
    def get_all_fsp_status(self) -> dict:
        """Get FSP status from all cells connected to the datacenter."""
        statuses = {}
    ...
    @classmethod
    def create_local(cls, state: str, init_topology_path: str, *args, **kwargs) -> "Datacenter":
        """Returns a datacenter with a preconfigured topology and support with more arguments"""
        dc = Datacenter()
        dc.state = state
        dc.load_topology(init_topology_path)
