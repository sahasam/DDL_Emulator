...
class SliceProtocol(LinkProtocol):
    """
    ...
    - LIVENESS: Keep-alive packet when no data is available
    """
    def __init__(self,
                 io: PortIO,
                 name: str,
    ...
    async def _process_send(self):
        ...
        3. If no data, send LIVENESS packet
        4. Transition to RA state to wait for ACK
        """
            ...
            while True:
                if self.state_machine.state == "RD":
                    ...
                    if not self.io.write_q.empty():
                        ...
                        # If no data, send LIVENESS packet
                        if self.is_client:
                            self.transport.sendto(b"LIVENESS")
                        else:
                            self.transport.sendto(b"LIVENESS", self.sending_addr)
                        self.logger.info("No data to send, sent LIVENESS packet")
